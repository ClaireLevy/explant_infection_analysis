...
Titrate lot1 of vNL_sNLuc_6ATRi.B.Bal.ecto based on the kinetics of RA and KW's virus; "Bal26 lot1", at the concentration that they use for infection experiments (1.92E6 pfu/ml)
...
ggplot(titer001_day1_14, aes(x = Day, y = RLU))+
  ...
  scale_shape_manual(values = c("Hladik Bal lot1" = 16, "Bal26 lot1" = 1, "No virus" = 4))+
  scale_y_log10()
...
Here are side-by-side plots from both titrations showing the Bal26 lot1 and the closest pfu/ml of the Hladik Bal lot1

```{r echo=FALSE, message=FALSE, warning=FALSE}

#want to plot the RLU for both titrations but just for the conditions that were closest to Bal26 lot1.

# combine the data  frames from titration001 day 1-21(conditions ACD) and titration 002 day 1-21 and then filter.


titer_001_002_d1_21<- list(titer001_day1_21ACD, titer002_day1_21)


names(titer_001_002_d1_21)<-c("titer001_day1_21ACD","titer002_day1_21")


titer_001_002_d1_21 <-bind_rows(titer_001_002_d1_21, .id = "Experiment")


#filter to get data for just No virus, Bal26 lot1, and Hladik Bal lot1 with dilution factors of 68.76, 46.3, 34.38

Bal26AndNoVirus <- titer_001_002_d1_21 %>%
  filter(Virus != "Hladik Bal lot1")

Hladik <- titer_001_002_d1_21 %>%
  filter(Virus == "Hladik Bal lot1")%>%
  filter(Dilution_factor ==46.3|Dilution_factor ==34.38)


both <- rbind(Bal26AndNoVirus,Hladik)

both$Day <-factor(both$Day, levels = c("1","1 wash", "3","6","7","8","10","13","14","15","17","21"))


ggplot(both, aes(x = Day, y = RLU))+
  geom_point(aes(color = pfu_per_ml, shape = Virus), size = 3)+
  geom_line(aes(group = pfu_per_ml, color = pfu_per_ml))+
  scale_shape_manual(values = c("Hladik Bal lot1" = 16, "Bal26 lot1" = 1, "No virus" = 4))+
  scale_y_log10()+
  facet_wrap(~Experiment)+
  ggtitle("Selected pfu/ml from both titrations")+
